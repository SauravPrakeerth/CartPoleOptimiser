...
from CartpoleNoise import CartPole, remap_angle
import random
import numpy
...
def alphaGenerator(N,M,lambd):
    ...
    # Adding noise in observed data #
    Xdata = gNoise(Xdata, 0, 1, True)
    Ydata_CL = gNoise(Ydata_CL, 0, 1)
    Ydata_CV = gNoise(Ydata_CV, 0, 1)
    Ydata_PA = gNoise(Ydata_PA, 0, 1)
    Ydata_PV = gNoise(Ydata_PV, 0, 1)
...
def gNoise(arr, mu, sig, mat = False):

    sh = [len(arr), len(arr[0])] if mat else [1,len(arr)]
    noise_mat = []
    for n in range(sh[0]):
        noise = np.random.normal(mu, sig, sh[1])
        noise_mat.append(noise)
    return arr + np.array(noise_mat)
