...
def _customLoss(state,sigma):
    return 1-np.exp(-np.dot(state/sigma,state/sigma)/(2.0))


def loss(state,sigma=np.array([1,1,1,1])):
    #return _customLoss(state,sigma)
    return _loss(state)
...
class CartPole:
    ...
    def __init__(self, nMean, nStd, visual=False):
    ...
    def setState(self, state):
        self.cart_location = state[0] + np.random.normal(self.nMean,self.nStd,1)[0]
        self.cart_velocity = state[1] + np.random.normal(self.nMean,self.nStd,1)[0]
        self.pole_angle = state[2] + np.random.normal(self.nMean,self.nStd,1)[0]
        self.pole_velocity = state[3] + np.random.normal(self.nMean,self.nStd,1)[0]
